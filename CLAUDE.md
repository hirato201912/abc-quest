...
| listening | きいて えらぶ: 音声で流れたアルファベットを4択から選ぶ（クイズの逆）。正解でことば＋絵のごほうび表示 |
...
- listening / ordering モードは実装済み（2026-07-06）。**既存DBにはmode制約の変更SQLが必要**（supabase/abc_quest_records.sql 末尾のalter文。未実行だと新モードの記録がすべて失敗する — ゲーム自体は動く）
